...
class BaseAppResponse(BaseResponse):
    ...
    key: str = Field(..., title="App Key", examples=["app1"],
                     description="Unique key/slug for the app, used in URLs and API references")
    name: str = Field(..., title="App Name", examples=["app1"], description="The name of the app")
    display_name: Optional[str] = Field(..., title="Display Name", examples=["App 1"],
                                        description="The display name of the app")
    description: str = Field(..., title="App Description", examples=["This is app1"],
                             description="The description of the app")
    logo: Optional[str] = Field(..., title="Logo", examples=["http://example.com/icon.png"],
                                description="The logo of the app")
    categories: Optional[list[str]] = Field(..., title="Categories", examples=[["category1", "category2"]],
                                            description="The categories of the app")
    enabled: Optional[bool] = Field(..., title="Enabled", examples=[True],
                                    description="Indicates if the app is enabled")
    created_at: Optional[str] = Field(..., title="Created At", examples=["2023-01-01T00:00:00Z"],
                                      description="The creation date of the app")
    updated_at: Optional[str] = Field(..., title="Updated At", examples=["2023-01-01T00:00:00Z"],
    ...
    no_auth: Optional[bool] = Field(..., title="No Auth", examples=[False],
...
class TestConnectorResponse(BaseResponse):
    id: str = Field(..., title="Test Connector ID", examples=["12345"],
                    description="Unique identifier (UUID) for the test connector")
    name: str = Field(..., title="Test Connector Name", examples=["Test Connector 1"],
                      description="The name of the test connector")
    auth_scheme: str = Field(
        ..., title="Test Connector Auth Scheme", examples=["oauth2"],
        description="The authentication schemes supported by the test connector"
    )
...
class BaseActionResponse(BaseResponse):
    ...
    display_name: str = Field(..., title="Action Display Name", examples=["Action 1"],
                              description="The display name of the action")
    description: str = Field(..., title="Action Description", examples=["This is action1"],
    ...
    deprecated: Optional[bool] = Field(
        ..., title="Deprecated", examples=[False],
        description="Whether the action is deprecated, if true, avoid using this action."
    )
...
class SingleActionResponse(BaseActionResponse):
    app_key: str = Field(
        ..., title="App Key", examples=["app1"],
        description="The name of the app that the action belongs to. This is same as app_id."
    )
    app_name: str = Field(..., title="App Name", examples=["app1"],
    ...
    available_versions: Optional[list[str]] = Field(
        ..., title="Available Versions", examples=[["1.0", "2.0"]],
        description="List of available versions of the action."
    )
    no_auth: bool = Field(..., title="No Auth", examples=[False],
...
class FilterComposioSingleAppResponse(BaseResponse):
    data: Optional[SingleAppResponse] = Field(..., title="App details",
                                              examples=[{}])  # Only None is returned when the app is not found
...
class FilterComposioAppEnumsResponse(BaseResponse):
    items: list[str] = Field(..., title="List of all apps enums", examples=[["GMAIL", "GOOGLE_CALENDAR"]])
...
class GetSingleActionComposioResponse(BaseResponse):
    success: bool = Field(..., title="Success", examples=[True])
    message: Optional[str] = Field(None, title="Message", examples=["Success"])
    data: Optional[SingleActionResponse] = Field(..., title="Action details",
                                                 examples=[{}])  # Only None is returned when the action is not found
...
class GetAllActionComposioEnumsResponse(BaseResponse):
    success: bool = Field(..., title="Success", examples=[True])
    message: Optional[str] = Field(None, title="Message", examples=["Success"])
    items: list[str] = Field(..., title="List of all actions enums",
                             examples=[["GMAIL_CREATE_EMAIL_DRAFT", "GITHUB_GET_THE_AUTHENTICATED_USER"]])
