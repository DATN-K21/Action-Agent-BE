...
@router.post(
    ...
    response_model=ResponseWrapper[CreateThreadResponse],
)
async def create_new_thread(
    ...
    auth_context.ensure_user_id(request.user_id)
    response = await thread_service.create_thread(request)
    return response.to_response()


@router.get(
    "/{user_id}/{thread_id}/get-detail",
    summary="Get the details of the given thread.",
...
@router.post(
    "/{user_id}/get-all",
    summary="Get all threads of the given user.",
...
@router.delete(
    "/{user_id}/{thread_id}/delete",
    summary="Delete the given thread.",
