...
import openai
import tiktoken
from g4f import Client, AsyncClient
...
def _convert_messages_to_dicts(messages: Sequence[BaseMessage]) -> List[dict]:
    """Convert a sequence of LangChain messages to dictionaries."""
...
class BaseChatG4F(BaseChatModel):
    ...
    g4f_proxy: Optional[str] = Field(
        default_factory=from_env("G4F_PROXY", default=None)
    )
    ...
    API but with different models. In those cases, in order to avoid errors 
    ...
    def _default_params(self) -> Dict[str, Any]:
        """Get the default parameters for calling G4F API."""
        exclude_if_none = {
...
class ChatG4F(BaseChatG4F):  # type: ignore[override]
    """G4F chat model integration."""
    ...
    def lc_secrets(self) -> Dict[str, str]:
        return {"g4f_api_key": "G4F_API_KEY"}

    @classmethod
    def get_lc_namespace(cls) -> List[str]:
        """Get the namespace of the langchain object."""
        return ["langchain", "chat_models", "gf4"]
    ...
    def lc_attributes(self) -> Dict[str, Any]:
        ...
        if self.g4f_proxy:
            attributes["g4f_proxy"] = self.g4f_proxy
    ...
    def with_structured_output(
        ...
        """Model wrapper that returns outputs formatted to match the given schema."""  # noqa: E501
...
def _oai_structured_outputs_parser(ai_msg: AIMessage) -> PydanticBaseModel:
    if ai_msg.additional_kwargs.get("parsed"):
        ...
        raise G4FRefusalError(ai_msg.additional_kwargs["refusal"])
    else:
        raise ValueError(
...
class G4FRefusalError(Exception):
