...
from app.core.tools.tool_manager import create_unique_key, tool_manager
...
async def load_skill_for_chatbot(member_id: str):
    async with AsyncSessionLocal() as session:
        try:
            # Get member
            statement = select(Member).select_from(Member).where(Member.id == member_id, Member.is_deleted.is_(False))
            ...
            # Get skill
            statement = select(Skill).select_from(Skill).where(Skill.id == skill_id, Skill.is_deleted.is_(False))
