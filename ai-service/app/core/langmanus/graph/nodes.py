...
from app.core.langmanus.tools import crawl_tool, get_retriever_tool, get_web_search_tool, python_repl_tool
...
def background_investigation_node(state: State, config: RunnableConfig):
    ...
    if SELECTED_SEARCH_ENGINE == SearchEngine.TAVILY.value:
        searched_content = LoggedTavilySearch(max_results=configurable.max_search_results).invoke(query)  # type: ignore
        ...
        background_investigation_results = get_web_search_tool(configurable.max_search_results).invoke(query)  # type: ignore
...
def planner_node(state: State, config: RunnableConfig) -> Command[Literal["human_feedback", "reporter"]]:
    ...
    messages = apply_prompt_template("planner", state, configurable)  # type: ignore
    ...
    if AGENT_LLM_MAP["planner"] == "basic":
        ...
        full_response = response.model_dump_json(indent=4, exclude_none=True)  # type: ignore
    else:
...
def coordinator_node(state: State, config: RunnableConfig) -> Command[Literal["planner", "background_investigator", "__end__"]]:
    ...
    configurable = Configuration.from_runnable_config(config)
    messages = apply_prompt_template("coordinator", state)  # type: ignore
...
def reporter_node(state: State):
    ...
    current_plan = state.get("current_plan")
    input_ = {
        "messages": [HumanMessage(f"# Research Requirements\n\n## Task\n\n{current_plan.title}\n\n## Description\n\n{current_plan.thought}")],  # type: ignore
        "locale": state.get("locale", "en-US"),
    }
    invoke_messages = apply_prompt_template("reporter", input_)  # type: ignore
...
async def _execute_agent_step(state: State, agent, agent_name: str) -> Command[Literal["research_team"]]:
    ...
    current_step = None
    completed_steps = []
    for step in current_plan.steps:  # type: ignore
    ...
    try:
        ...
        logger.warning(f"Invalid AGENT_RECURSION_LIMIT value: '{raw_env_value}'. Using default value {default_recursion_limit}.")
        recursion_limit = default_recursion_limit
    ...
    if configurable.mcp_settings:
        for server_name, server_config in configurable.mcp_settings["servers"].items():
            if server_config["enabled_tools"] and agent_type in server_config["add_to_agents"]:
                mcp_servers[server_name] = {k: v for k, v in server_config.items() if k in ("transport", "command", "args", "url", "env")}
